...
async (conn, mek, m, { from, q, reply, l }) => {
    try {
        ...
        let txt = `*🗃️ 딜리샤 로드 GDRIVE DOWNLOADER 🗃️*
...
async(conn, mek, m, {
  ...
  try {
    ...
    let caption = `\`*🎧 딜리샤 로드 SONG DOWNLOADER 🎧*\`	
*┌────────────────────┐*
*├ \`🎶 Title\` : ${result.title}*
*├ \`🐼 Views\` : ${result.views}*
...
async (conn, mek, m, {
  ...
  try {
    ...
    let caption = `🎶 *킹_딜리샤 SONG DOWNLOADER* 🎶
...
const buttons = [
	{buttonId: prefix + 'dilisha ' + result.url, buttonText: {displayText: 'Send info 🎶'}, type: 1},
  {buttonId: prefix + 'dilo ' + result.url, buttonText: {displayText: 'Audio Type 🎶'}, type: 1}
...
let caption = `*\`🎬 킹_딜리샤 TIKTOK DOWNLOADER 🎬\`*
...
cmd({
    ...
    try {
        ...
        const caption = `\`🏮 킹_딜리샤 FB DOWNLOADER 🏮\`\n\n` +
                   `*┌──────────────────*\n` +
...
async(conn, mek, m,{from, q, reply}) => {
  try {
    ...
    let listdata = `*\`🗃️ 킹_딜리샤 APK DOWNLOADER 🗃️\`*
...
try{            
    ...
    let wm = config.FOOTER
    var result = results.videos[0]
     let caption = `*📹 킹_딜리샤 VIDEO DOWNLOADER 📹*
*┌─────────────────────┐*
*├ \`📹 Title\` : ${result.title}* 
*├ \`🐼 Views\` : ${result.views}*
*├ \`⌛Duration\` : ${result.duration}*
...
let listdata = `*\`🔥 킹_딜리샤 MEDIAFIRE DOWNLOADER 🔥\`*
...
let msg = `*\`🔞 킹_딜리샤 PORNHUB DOWNLOADER 🔞\`*
...
async (conn, mek, m, { from, prefix, l, quoted, body, isCmd, command, args, q, isGroup, sender, senderNumber, botNumber2, botNumber, pushname, isMe, isOwner, groupMetadata, groupName, participants, groupAdmins, isBotAdmins, isAdmins, reply }) => {
    try {
        ...
        let caption = `*\`🎼 킹_딜리샤 SPORTIFY DOWNLOADER 🎼\`*
*┌──────────────────╮*
*├ \`🎶 Title:\`* ${data.title}
*├ \`🧑‍🎤 Artist:\`* ${data.artist}
*├ \`💽 Album:\`* ${data.album}
...
async (conn, mek, m, { from, prefix, q, reply }) => {
    try {
        ...
        const caption = `*\`🎼 킹_딜리샤 SOUNDCLOUD DOWNLOADER 🎼\`*\n\n*🎶 Title:* ${data.title}\n*🔗 URL:* ${q}`;
        ...
        const buttonMessage = {
            ...
            caption: caption,
            footer: config.FOOTER || '킹_딜리샤u BOT',
            buttons: buttons,
            headerType: 4
        };
